...
import { getOperationsStaffs } from 'apiSdk/operations-staffs';
...
function DashboardPage() {
  ...
  const operationsStaffStat = useSWR<PaginatedInterface<OperationsStaffInterface>>(
    () => `/operations-staffs`,
    () => getOperationsStaffs({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: operationsStaffStat?.data?.totalCount,
